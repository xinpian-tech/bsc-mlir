...
mod tests {
    use super::*;
    use bsc_syntax::{IKind, IType, ITyCon, ITyConInfo, ITySort};
    use num_bigint::BigInt;

    #[test]
    fn test_export_kind_star() {
        let kind = IKind::Star;
        let json = export_kind(&kind);
        assert_eq!(json, json!({"tag": "KStar"}));
    }

    #[test]
    fn test_export_kind_fun() {
        let kind = IKind::Fun(Box::new(IKind::Star), Box::new(IKind::Num));
        let json = export_kind(&kind);
        assert_eq!(
            json,
            json!({
    ...
    #[test]
    fn test_export_type_num_literal() {
        let ty = IType::Con(ITyCon::Num(BigInt::from(42)));
        let json = export_type(&ty);
        assert_eq!(
            json,
            json!({
                "tag": "ITCon",
    ...
    #[test]
    fn test_export_type_gen() {
        let ty = IType::Gen(3);
        let json = export_type(&ty);
        assert_eq!(json, json!({"tag": "ITGen", "index": 3}));
    }

    #[test]
    fn test_export_options_default() {
        let opts = ExportOptions::default();
        assert!(!opts.include_positions);
        assert!(!opts.pretty);
    }

    #[test]
    fn test_export_def_prop() {
        assert_eq!(export_def_prop(&DefProp::NoInline), json!({"tag": "NoInline"}));
        assert_eq!(export_def_prop(&DefProp::AlwaysInline), json!({"tag": "AlwaysInline"}));
        assert_eq!(export_def_prop(&DefProp::Arity(3)), json!({"tag": "Arity", "value": 3}));
        assert_eq!(export_def_prop(&DefProp::Primitive), json!({"tag": "Primitive"}));
    }

    // TODO: test_export_verilog_dir when IVerilogDir is implemented
}
