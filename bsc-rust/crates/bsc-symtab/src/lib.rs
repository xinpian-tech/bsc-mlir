...
mod tests {
    ...
    fn make_id(name: &str) -> Id {
        Id::new(name, Position::unknown())
    }

    fn make_dummy_type() -> Type {
        Type::con(TyCon::abstract_type(make_id("Int"), Kind::Star))
    }

    fn make_scheme() -> Scheme {
        Scheme::mono(make_dummy_type())
    }

    fn make_assump(name: &str) -> Assump {
        Assump::new(make_id(name), make_scheme())
    }

    #[test]
    fn test_empty_symtab() {
        let st = SymTab::new();
        assert!(st.is_empty());
        assert_eq!(st.var_count(), 0);
        assert_eq!(st.type_count(), 0);
    }

    #[test]
    fn test_add_lookup_var() {
        let mut st = SymTab::new();
        let id = make_id("foo");
        let info = VarInfo::new(VarKind::Defn, make_assump("foo"));

        st.add_var(id.clone(), info.clone());

        assert!(st.has_var(&id));
        let found = st.lookup_var(&id).unwrap();
        assert_eq!(found.kind, VarKind::Defn);
    }

    #[test]
    fn test_add_lookup_type() {
        let mut st = SymTab::new();
        let id = make_id("MyType");
        let info = TypeInfo::abstract_type(id.clone(), Kind::Star);

        st.add_type(id.clone(), info);

        assert!(st.has_type(&id));
        let found = st.lookup_type(&id).unwrap();
        assert_eq!(found.kind, Kind::Star);
        assert!(found.is_abstract());
    }

    #[test]
    fn test_add_lookup_con() {
        let mut st = SymTab::new();
        let con_id = make_id("MyCon");
        let type_id = make_id("MyType");
        let tag_info = ConTagInfo::new(0, 2);
        let info = ConInfo::new(type_id, true, make_assump("MyCon"), tag_info);

        st.add_con(con_id.clone(), info);

        assert!(st.has_con(&con_id));
        let found = st.lookup_con(&con_id).unwrap();
        assert_eq!(found.len(), 1);
        assert!(found[0].visible);
    }

    #[test]
    fn test_add_lookup_field() {
        let mut st = SymTab::new();
        let field_id = make_id("myField");
        let type_id = make_id("MyStruct");
        ...
        st.add_field(field_id.clone(), info);

        assert!(st.has_field(&field_id));
        let found = st.lookup_field(&field_id).unwrap();
        assert_eq!(found.len(), 1);
        assert!(found[0].visible);

        // Test lookup_field_in_type
        let found2 = st.lookup_field_in_type(&field_id, &type_id).unwrap();
        assert_eq!(&found2.name, &field_id);
    }

    #[test]
    fn test_add_lookup_class() {
        let mut st = SymTab::new();
        let class_id = make_id("MyClass");
        let param = make_id("a");
        let info = ClassInfo::new(vec![], param, vec![make_id("method1")]);

        st.add_class(class_id.clone(), info);

        assert!(st.has_class(&class_id));
        let found = st.lookup_class(&class_id).unwrap();
    ...
    fn test_merge() {
        ...
        // Add var to st1
        let var_id = make_id("var1");
        st1.add_var(
            var_id.clone(),
            VarInfo::new(VarKind::Defn, make_assump("var1")),
        );

        // Add type to st2
        let type_id = make_id("Type1");
        st2.add_type(
            type_id.clone(),
            TypeInfo::abstract_type(type_id.clone(), Kind::Star),
        );

        // Merge
        st1.merge(st2);

        // Both should be in st1 now
        assert!(st1.has_var(&var_id));
        assert!(st1.has_type(&type_id));
    }

    #[test]
    fn test_con_visibility_override() {
        let mut st = SymTab::new();
        let con_id = make_id("MyCon");
        let type_id = make_id("MyType");
        let tag_info = ConTagInfo::new(0, 1);

        // Add hidden constructor first
        let hidden = ConInfo::new(type_id.clone(), false, make_assump("MyCon"), tag_info);
        st.add_con(con_id.clone(), hidden);

        // Add visible constructor with same signature
        let visible = ConInfo::new(type_id.clone(), true, make_assump("MyCon"), tag_info);
        st.add_con(con_id.clone(), visible);

        // Should have one entry, and it should be visible
        let found = st.lookup_con(&con_id).unwrap();
        assert_eq!(found.len(), 1);
        assert!(found[0].visible);
    }
    ...
    #[test]
    fn test_type_abstract_override() {
        let mut st = SymTab::new();
        let type_id = make_id("MyType");

        // Add abstract type first
        let abstract_info = TypeInfo::abstract_type(type_id.clone(), Kind::Star);
        st.add_type(type_id.clone(), abstract_info);

        // Add concrete type
        let concrete_info = TypeInfo::data_type(
            type_id.clone(),
            Kind::Star,
            vec![],
            vec![make_id("Con1")],
        ...
        st.add_type(type_id.clone(), concrete_info);

        // Should be concrete now
        let found = st.lookup_type(&type_id).unwrap();
        assert!(!found.is_abstract());
    }

    #[test]
    fn test_con_tag_info() {
        let tag = ConTagInfo::new(0, 4);
        assert_eq!(tag.con_no, 0);
        assert_eq!(tag.num_con, 4);
        assert_eq!(tag.con_tag, 0);
        assert_eq!(tag.tag_size, 2); // ceil(log2(4)) = 2

        let tag2 = ConTagInfo::new(2, 3);
        assert_eq!(tag2.tag_size, 2); // ceil(log2(3)) = 2

        let tag3 = ConTagInfo::new(0, 1);
        assert_eq!(tag3.tag_size, 0); // single constructor needs no tag
        assert!(tag3.is_single());
    }

    #[test]
    fn test_class_with_instances() {
        use bsc_types::Typeclass;

        let mut class_info = ClassInfo::new(vec![], make_id("a"), vec![make_id("show")]);

        let instance = Instance::new(
            vec![],
            Pred::new(Typeclass::new(make_id("Show")), vec![make_dummy_type()]),
        );
        class_info.add_instance(instance);

        assert_eq!(class_info.instances.len(), 1);
    }

    #[test]
    fn test_symtab_display() {
        let mut st = SymTab::new();
        st.add_var(
            make_id("x"),
            VarInfo::new(VarKind::Defn, make_assump("x")),
        );
        st.add_type(
            make_id("T"),
            TypeInfo::abstract_type(make_id("T"), Kind::Star),
        );

        let display = format!("{st}");
        assert!(display.contains("vars: 1 entries"));
        assert!(display.contains("types: 1 entries"));
    }

    #[test]
    fn test_scheme_display() {
        let mono = Scheme::mono(make_dummy_type());
        assert!(!format!("{mono}").contains("forall"));

        let poly = Scheme::forall(
            vec![Kind::Star, Kind::Num],
            QualType::unqualified(make_dummy_type()),
        );
    ...
    fn test_type_info_helpers() {
        let struct_info = TypeInfo::struct_type(
            make_id("MyStruct"),
            Kind::Star,
            vec![],
            vec![make_id("field1"), make_id("field2")],
        );

        assert!(!struct_info.is_interface());
        assert_eq!(struct_info.fields().unwrap().len(), 2);
        assert!(struct_info.constructors().is_none());

        let data_info = TypeInfo::data_type(
            make_id("MyData"),
            Kind::Star,
            vec![],
            vec![make_id("Con1"), make_id("Con2")],
            false,
        );

        assert!(data_info.fields().is_none());
        assert_eq!(data_info.constructors().unwrap().len(), 2);
    }
}
