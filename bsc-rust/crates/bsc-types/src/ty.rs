...
mod tests {
    ...
    fn test_kind_display() {
        ...
        assert_eq!(format!("{}", Kind::Num), "#");
        assert_eq!(format!("{}", Kind::Str), "$");
        assert_eq!(format!("{}", Kind::fun(Kind::Star, Kind::Star)), "* -> *");
        assert_eq!(
            format!("{}", Kind::fun(Kind::fun(Kind::Star, Kind::Star), Kind::Num)),
            "(* -> *) -> #"
        );
    }

    #[test]
    fn test_kind_arg_result() {
        let k = Kind::fun(Kind::Star, Kind::fun(Kind::Num, Kind::Str));
        let args = k.arg_kinds();
        assert_eq!(args.len(), 2);
        assert_eq!(args[0], &Kind::Star);
        assert_eq!(args[1], &Kind::Num);
        assert_eq!(k.result_kind(), &Kind::Str);
    }

    #[test]
    fn test_tyvar_equality() {
        let tv1 = TyVar::new(Id::unpositioned("a"), Kind::Star);
        let tv2 = TyVar::new(Id::unpositioned("a"), Kind::Num); // Different kind
        let tv3 = TyVar::new(Id::unpositioned("b"), Kind::Star);

        // Same name, different kind -> equal (kind ignored in equality)
        assert_eq!(tv1, tv2);
    ...
    #[test]
    fn test_tycon_num() {
        let tc = TyCon::num_i64(42, Position::unknown());
        assert!(tc.is_num());
        assert!(!tc.is_str());
        assert_eq!(tc.kind(), Some(&Kind::Num));
    }

    #[test]
    fn test_type_split_app() {
        // Build: F a b
        let f = Type::con(TyCon::Named {
            name: Id::unpositioned("F"),
            kind: Some(Kind::fun(Kind::Star, Kind::fun(Kind::Star, Kind::Star))),
            sort: TyConSort::Abstract,
        });
        let a = Type::var(TyVar::new(Id::unpositioned("a"), Kind::Star));
        let b = Type::var(TyVar::new(Id::unpositioned("b"), Kind::Star));

        let ty = Type::app(Type::app(f.clone(), a.clone()), b.clone());

        let (head, args) = ty.split_app();
        assert_eq!(head, &f);
        assert_eq!(args.len(), 2);
    }
    ...
    #[test]
    fn test_type_num() {
        let ty = Type::num_i64(123, Position::unknown());
        assert!(ty.is_num());
        assert_eq!(ty.get_num_i64(), Some(123));
        assert_eq!(ty.kind(), Some(Kind::Num));
    }

    #[test]
    fn test_type_num_bigint() {
        use num_bigint::BigInt;
        let big_value = BigInt::from(i128::MAX) + BigInt::from(1);
        let ty = Type::num(big_value.clone(), Position::unknown());
        assert!(ty.is_num());
        assert_eq!(ty.get_num(), Some(&big_value));
        // get_num_i64 should return None since it doesn't fit
        assert_eq!(ty.get_num_i64(), None);
    }
}
