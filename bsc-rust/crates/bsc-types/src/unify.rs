...
mod tests {
    ...
    fn make_num_var(name: &str) -> TyVar {
        TyVar::new(Id::unpositioned(name), Kind::Num)
    }
    ...
    fn make_num_type_var(name: &str) -> Type {
        Type::var(make_num_var(name))
    }

    #[test]
    fn test_unify_same_var() {
        let t = make_type_var("a");
        let result = unify(&[], &t, &t);
        assert!(result.is_ok());
        let (subst, eqs) = result.unwrap();
    ...
    fn test_unify_var_with_type() {
        let var_a = make_type_var("a");
        let ty_int = Type::num(BigInt::from(42), Position::unknown());

        let result = unify(&[], &var_a, &ty_int);
        assert!(result.is_ok());
        let (subst, _) = result.unwrap();

        let a = make_var("a");
        assert_eq!(subst.apply(&var_a), ty_int);
    }

    #[test]
    fn test_unify_different_nums_fails() {
        let t1 = Type::num(BigInt::from(1), Position::unknown());
        let t2 = Type::num(BigInt::from(2), Position::unknown());
    ...
    fn test_unify_app() {
        ...
        let var_a = make_type_var("a");
        let ty_int = Type::con(TyCon::Named {
            name: Id::unpositioned("Int"),
            kind: Some(Kind::Star),
            sort: TyConSort::Abstract,
        });

        let t1 = Type::app(f.clone(), var_a.clone());
        let t2 = Type::app(f, ty_int.clone());
    ...
    fn test_occurs_check() {
        // Try to unify a with (F a) - should fail
        let f = Type::con(TyCon::Named {
            name: Id::unpositioned("F"),
            kind: Some(Kind::fun(Kind::Star, Kind::Star)),
            sort: TyConSort::Abstract,
        ...
        let var_a = make_type_var("a");
        let t2 = Type::app(f, var_a.clone());

        let result = unify(&[], &var_a, &t2);
        assert!(matches!(result, Err(UnifyError::OccursCheck { .. })));
    }

    #[test]
    fn test_numeric_unification_deferred() {
        // Unify numeric type variables - may be deferred
        let n1 = make_num_type_var("n");
        let n2 = make_num_type_var("m");

        let result = unify(&[], &n1, &n2);
        assert!(result.is_ok());
    }
    ...
    fn test_match_type() {
        ...
        // a matches Int
        let result = match_type(&var_a, &ty_int);
        assert!(result.is_ok());

        // Int does not match a (matching is one-way)
        let result = match_type(&ty_int, &var_a);
        assert!(result.is_err());
    }

    #[test]
    fn test_bound_var_conflict() {
        let var_a = make_var("a");
        let var_b = make_var("b");

        let ta = Type::var(var_a.clone());
        let tb = Type::var(var_b.clone());

        // Both bound - cannot unify
        let result = unify(&[var_a.clone(), var_b.clone()], &ta, &tb);
        assert!(matches!(result, Err(UnifyError::BoundVarConflict { .. })));
    }
}
